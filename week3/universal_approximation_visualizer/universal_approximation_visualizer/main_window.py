...
from approximation_widget import ApproximationWidget
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("범용 근사 학습 도구 v1.0")
        ...
        header = QWidget(); header.setFixedHeight(58)
        header.setStyleSheet("background: #1a2634;")
        hl = QHBoxLayout(header); hl.setContentsMargins(20, 0, 20, 0)
        logo = QLabel("🌊 범용 근사 학습 도구")
        logo.setStyleSheet("color: white; font-size: 18px; font-weight: bold;")
        sub = QLabel("Universal Approximation Visualizer — Week 3")
        sub.setStyleSheet("color: #7f8c8d; font-size: 12px;")
        hl.addWidget(logo); hl.addStretch(); hl.addWidget(sub)
        root.addWidget(header)

        tabs = QTabWidget(); tabs.setDocumentMode(True)
        tabs.setStyleSheet("""
            QTabWidget::pane{border:none;background:#f4f6f8}
            QTabBar::tab{padding:10px 24px;font-size:13px;font-weight:bold;
                background:#dfe6e9;color:#636e72;border:none;border-bottom:3px solid transparent}
        ...
        tabs.addTab(ApproximationWidget(),    "🎯 함수 근사")
        tabs.addTab(ComparisonWidget(),       "📊 뉴런 수 비교")
        root.addWidget(tabs, 1)

        footer = QWidget(); footer.setFixedHeight(28)
        footer.setStyleSheet("background: #f0f4f8;")
        fl = QHBoxLayout(footer); fl.setContentsMargins(16, 0, 16, 0)
        fl.addWidget(QLabel("학습 순서: 이론 설명 → 함수 근사 → 뉴런 수 비교"))
        fl.addStretch(); fl.addWidget(QLabel("Week 3 — Universal Approximation"))
